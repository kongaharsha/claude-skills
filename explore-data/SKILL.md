...
3. For Excel workbooks, prefer Python-based profiling with `pandas`/`openpyxl`, especially for multi-sheet files or when you need repeatable cleanup logic.
4. If the workbook is stored in OneDrive or another cloud-synced directory, copy it to a local temp path before reading it with Python libraries. This avoids direct-read failures caused by placeholders, sync state, or locks.
5. If Python still cannot read the workbook, use Excel COM or another spreadsheet-native path as a fallback for inspection.
