...
2. For Excel workbooks, prefer Python-based extraction with `pandas`/`openpyxl` over manual spreadsheet inspection when the task involves repeatable logic, cleanup, joins, or multi-sheet analysis.
3. If the Excel file lives in OneDrive or another cloud-synced folder, first copy it to a local temp path before reading it with Python libraries. Direct reads can fail due to sync placeholders or file locks even when Excel itself opens the file.
4. Use Excel COM only as a fallback for inspection when Python cannot read the workbook or when you need behavior that matches the Excel desktop app.
5. If writing queries for manual execution, use the `sql-queries` skill for dialect-specific best practices
6. Once data is provided, proceed with analysis
