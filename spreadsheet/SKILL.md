...
2. For analysis of `.xlsx` files, prefer Python (`pandas` plus `openpyxl`) over manual spreadsheet inspection when the task involves cleanup rules, deduplication, aggregation, or reusable extraction logic.
3. If the workbook is in OneDrive or another cloud-synced directory, copy it to a local temp file before reading it with Python libraries. Excel desktop may open the original while direct library reads fail.
4. Prefer `openpyxl` for `.xlsx` editing and formatting. Use `pandas` for analysis and CSV/TSV workflows.
5. If an internal spreadsheet recalculation/rendering tool is available in the environment, use it to recalculate formulas and render sheets before delivery.
6. Use formulas for derived values instead of hardcoding results.
7. If layout matters, render for visual review and inspect the output.
8. Save outputs, keep filenames stable, and clean up intermediate files.
...
- On Windows/OneDrive paths, prefer reading from a temp local copy when using `pandas` or `openpyxl`.
